...
//#define M5STACK_PSRAM
#define M5STACK_NOPSRAM

#warning "Compiling for M5STACK"
...
// SD_MMC does not work on ESP32-S2 / ESP32-C3 / M5STACK
...
int m5avail() { return((digitalRead(KEYBOARD_INT) == LOW) || (Serial.available())); }
...
void m5write(uint8_t *buf, int len)
{
  if(buf != NULL)
    for(int i=0; i<len; i++) tb_display_print_char(buf[i]);
}

#define REQUIRED_M5_SUPPORT \
  XV(m5stack, "m5write", M5_WRITE, m5write(b1, n0); DROPn(2)) \
  XV(m5stack, "m5key", M5_KEY, PUSH m5getch()) \
  XV(m5stack, "m5key?", M5_AVAIL, PUSH m5avail())
...
void setup() {
  M5.Power.begin();
  M5.Lcd.begin();
  M5.Lcd.setTextSize(2);  // One up from the smallest size
  M5.Lcd.setTextColor(GREEN, BLACK);
  M5.Lcd.setTextWrap(true, false);
  Wire.begin();           // Needed for the M5 Faces QWERTY keyboard.
  pinMode(KEYBOARD_INT, INPUT_PULLUP);
  //mySerial.begin(115200);
  tb_display_init(M5.Lcd.getRotation());
